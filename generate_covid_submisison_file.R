...
covid_hosps <- dfall %>%
  ...
  # Number of admissions plot just for US
    covid_hosps_US <- full_join( dfall, hhs_sub, by=c("state","date")) %>%
    filter(date >= "2022-01-01") %>%
    filter(state == "US") %>%
    ggplot() +
    geom_ribbon( aes(date, ymin= lower_95, ymax=upper_95), alpha=1.0,col='blue') +
    geom_line( aes(date, pred), alpha = 1.0) +
    geom_point( aes(date, covid), alpha=1.0) +
    facet_wrap(~state,scales="free")  +
    labs(x="", y="covid admissions") + scale_x_date(date_breaks = '1 month', date_labels = '%b') +
    theme(panel.background = element_rect(fill='white', colour='white'),
          plot.background =element_rect(fill='white', colour='white'))

  ggsave(filename = "covid_hosps_US.png",covid_hosps_US,device = "png",height = 5,width = 8)


  # Number of admissions plot in the Shape of the US
  covid_hosps_USShape <- full_join( dfall, hhs_sub, by=c("state","date")) %>%
  filter(date >= "2022-01-01") %>%
  ggplot() +
  geom_ribbon( aes(date, ymin= lower_95, ymax=upper_95), alpha=0.5,col='blue') +
  geom_line( aes(date, pred)) +
  geom_point( aes(date, covid), alpha=0.20) +
  geofacet::facet_geo(~state,grid="us_state_grid2", scales="free") +
  labs(x="", y="covid admissions") +
  scale_x_date(date_breaks = '3 month', date_labels = "%b") +
  theme(panel.background = element_rect(fill='white', colour='white'),
        plot.background =element_rect(fill='white', colour='white') )

  ggsave(filename = "covid_hosps_USShape.png",covid_hosps_USShape,device = "png",height = 10,width = 16)
